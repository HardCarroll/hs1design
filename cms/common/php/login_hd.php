...
if (isset($_POST["uid"]) && !empty($_POST["uid"])) {
  ...
  if(isset($_SESSION["user"]) && ($_SESSION["user"]["uid"]===$uid || $_SESSION["user"]["username"]===$uid)) {
    ...
    if ($_SESSION["user"]["password"] === sha1($pwd)) {
      ...
      header("location: /cms/admin/login.php?uid=$uid");
      exit;
    }
  }
  else {
    ...
    if ($dbo->state["err_no"]) {
      // 数据库连接失败跳转至登录界面
      header("location: /cms/admin/login.php?token=".$dbo->state["err_code"]);
      exit;
    }
...
//      验证成功
  ...
  header("location: /cms/admin/login.php?token=login");
  exit;
}
