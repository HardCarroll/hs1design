$(function(){
  ...
  $("#btn_consult").on("click", function() {
    ...
    $(this).attr('disabled', true).popover({
      trigger: 'manual',
      placement: 'top',
      html: true,
      title: 'tips',
      content: "type: "+type+"; area: "+area+"; phone: "+phone}).popover("show").on('shown.bs.popover', function(){
      setTimeout(function(){$("#btn_consult").popover('hide').attr('disabled', false)}, 3000);
    });
    
    var fmdConsult = new FormData();
    fmdConsult.append("type", type);
    fmdConsult.append("area", area);
    fmdConsult.append("phone", phone);
    $.ajax({
